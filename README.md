...
- [`SPRINT_INDEX.md`](./SPRINT_INDEX.md) - индекс спринтов этого display-репо
- [`SUPER_SPRINT_LOG.md`](./SUPER_SPRINT_LOG.md) - текущий супер-спринт display-слоя
...
3. step/event stream выполнения;
...
Если проектный sprint source отсутствует, UI должен показывать empty state, а не подставлять agent-owned governance batch.

## Как использовать

- UI может читать отсюда фиксированные примеры payload'ов или готовые read-models.
- Агент может быстро брать отсюда короткий контекстный prompt, чтобы не объяснять всё с нуля.
- Product index может ссылаться на этот репозиторий как на display/source layer.

## Важные правила

- Не смешивать сюда runtime backend-код.
